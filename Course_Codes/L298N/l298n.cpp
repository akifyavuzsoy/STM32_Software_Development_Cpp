...
L298N::L298N(pinStruct_t& motorPin1, pinStruct_t& motorPin2, pinStruct_t& pwmPin, TIM_TypeDef* TIMx, uint8_t gpioAFSelTIMx, uint8_t pwmChannel)
{
	Mpin1 = motorPin1;
	Mpin2 = motorPin2;
	pwmCh = pwmChannel;
  ...
  HAL_GPIO_Init(pwmPin.port,&GPIO_PWMInitStruct);	
	...
	GPIO_Pin1InitStruct.Pin = Mpin1.selectedPin;
  ...
  GPIO_Pin1InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(Mpin1.port,&GPIO_Pin1InitStruct);
	
	//GPIO config (motor pin2)
	GPIO_InitTypeDef GPIO_Pin2InitStruct = {0};
	GPIO_Pin2InitStruct.Pin = Mpin2.selectedPin;
  ...
  GPIO_Pin2InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(Mpin2.port,&GPIO_Pin2InitStruct);	
...
void L298N::MoveForward(uint32_t speed)
{
	__HAL_TIM_SET_COMPARE(&htim,pwmCh,speed);
	HAL_GPIO_WritePin(Mpin1.port,Mpin1.selectedPin,GPIO_PIN_SET);
	HAL_GPIO_WritePin(Mpin2.port,Mpin2.selectedPin,GPIO_PIN_RESET);
}

void L298N::Reverse(uint32_t speed)
{
	__HAL_TIM_SET_COMPARE(&htim,pwmCh,speed);
	HAL_GPIO_WritePin(Mpin1.port,Mpin1.selectedPin,GPIO_PIN_RESET);
	HAL_GPIO_WritePin(Mpin2.port,Mpin2.selectedPin,GPIO_PIN_SET);
}
